...
sql_tplt = f"""select company, count() as pr_create_count
from (select id, b.company
      from (select * from github_pull_requests where search_key__owner = '{owner}' and search_key__repo = '{repo}') as a global
