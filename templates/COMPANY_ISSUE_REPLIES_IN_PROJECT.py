...
def generate(owner, repo, email_domain_list):
    ...
    return f"""select company, count() as issues_comment_count
          ...
          from (select a.user__id
                from (select * from github_issues_comments) as a global
                         join (select search_key__owner, search_key__repo, number
                              ...
                              on a.search_key__owner = b.search_key__owner and a.search_key__repo = 
                              b.search_key__repo and
...
######################################
ID = 'COMPANY_ISSUE_REPLIES_IN_PROJECT'
TEMPLATE_INFO = {
    "label": "各公司在项目PR中回复的数量",
    "template_id": ID,
    "params": [
        {
